...
import static wingo.go.GoColor.BLACK_WHITE;
    ...
    */
    ...
    private static class StackEntry
    {
        ...
        public void execute(Board board)
        {
            ...
            if (p != null)
            {
                m_oldColor = board.getColor(p);//p為黑or白
                board.setColor(p, c);
                assert c != EMPTY;
    ...
    {
        assert m_mark.isCleared();//當標記被清除回傳true
        GoColor color = getColor(point);//取得點的顏色
        assert color != EMPTY;//顏色不為空值
        m_checkKillStack.clear();//清空stack
