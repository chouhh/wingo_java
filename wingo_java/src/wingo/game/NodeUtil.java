...
public final class NodeUtil
{
    ...
    */
    public static String nodeInfo(ConstNode node)
    {
        ...
        buffer.append("節點資訊:\n");
        appendInfo(buffer, "步數", getDepth(node));
        appendInfo(buffer, "子分支", node.getNumberChildren());
        if (node.getMove() != null)
        {
            appendInfo(buffer, "當前落子", node.getMove().toString());
            appendInfo(buffer, "移動步數", getMoveNumber(node));
        }
        appendInfo(buffer, "分支", getVariationString(node));
        ConstPointList black = node.getSetup(BLACK);
        if (black.size() > 0)
            appendInfo(buffer, "增加黑子", black);
        ConstPointList white = node.getSetup(WHITE);
        if (white.size() > 0)
            appendInfo(buffer, "增加白子", white);
        ConstPointList empty = node.getSetup(EMPTY);
        if (empty.size() > 0)
            appendInfo(buffer, "去除棋子", empty);
        if (node.getPlayer() != null)
            appendInfo(buffer, "棋士", node.getPlayer().toString());
        ...
        for (MarkType type : EnumSet.allOf(MarkType.class))
        {
            ...
            if (marked != null && marked.size() > 0)
                appendInfo(buffer, "標記 " + type, marked);
        }
        ...
        if (labels != null && ! labels.isEmpty())
        {
            ...
            //appendInfo(buffer, "標籤", labelsBuffer.toString());
        }
        ...
        if (info != null)
        {
            ...
            for (StringInfoColor type : EnumSet.allOf(StringInfoColor.class))
            {
                ...
                if (s != null)
                    appendInfo(buffer, type.toString() + "[黑方]", s);
                s = info.get(type, WHITE);
                if (s != null)
                    appendInfo(buffer, type.toString() + "[白方]", s);
            }
            if (info.getHandicap() != 0)
                appendInfo(buffer, "讓子", info.getHandicap());
            if (info.getKomi() != null)
                appendInfo(buffer, "貼目", info.getKomi().toString());
            if (info.getTimeSettings() != null)
                appendInfo(buffer, "對弈時間",
    ...
    */
    ...
    private static void appendInfoComment(StringBuilder buffer, ConstNode node)
    {
        ...
        if (comment != null)
            appendInfo(buffer, "遊戲結果", comment);
    }
